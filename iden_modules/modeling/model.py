"""
Main UA_VLFM modeling function.
"""
import torch
import torchvision
import numpy as np
...
class CLIPRModel(torch.nn.Module):
    def __init__(self, vision_type='resnet_v1', bert_type='emilyalsentzer/Bio_ClinicalBERT', vision_pretrained=True,
                 ...
                 out_path=None, image_size=256, caption="An OCT image of [CLS]", projection=True,
    ...
    def load_from_pretrained(self, weights_path=None):
        state_dict = torch.load(weights_path)
        weights_state_dict = self.state_dict()
        new_dict = {}
        for k in weights_state_dict.keys():
            if k in state_dict:
                new_dict[k] = state_dict[k]
            else:
                new_dict[k] = weights_state_dict[k]
    ...
    def preprocess_image(self, image):
        ...
        # 检查图像是否已经是张量，如果是则跳过 to_tensor 步骤
            lambda x: x if isinstance(x, torch.Tensor) else transforms.ToTensor(),
...
class Model_Finetuing(torch.nn.Module):
    def __init__(self,model_name,class_num,weight_path,IsFinetuning,R):
        ...
        RetiZeroModel_trained = CLIPRModel(vision_type=model_name, from_checkpoint=True, bert_type="./BioBert",
